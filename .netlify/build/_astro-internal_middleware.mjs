...
import 'clsx';
import { g as getSession } from './chunks/server_Bm-mB75U.mjs';
import { a as apiContextRoutesSymbol, g as getActionContext } from './chunks/server_CRdQ8Qgm.mjs';
...
const onRequest$2 = defineMiddleware(async ({ url, locals, redirect, request }, next) => {
  const session = await getSession(request);
  const isLoggedIn = !!session;
  const user = session?.user;
  locals.isLoggedIn = isLoggedIn, locals.user = null;
  locals.isAdmin = false;
  if (user) {
    locals.user = {
      name: user.name,
      // With the exclamation character we tell that always have a email and a password.
      email: user.email
    };
